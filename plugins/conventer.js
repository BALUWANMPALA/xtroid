    ...
    XTroid.addCMD({pattern: 'vsticker$', desc: Lang.ANİM_STİCK, fromMe: true}, (async (message, match) => {
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                const saving = data.pipe(fs.createWriteStream('/root/xtroid/stweb.mp4'))
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/xtroid/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: 'Made by Xtroid', quoted: message.data })
                    if (fs.existsSync(savedFilename)) fs.unlinkSync(savedFilename)
                    if (fs.existsSync('/root/xtroid/stweb.mp4')) fs.unlinkSync('/root/xtroid/stweb.mp4')
                })
            })
        })
    }));
